...
class ExportPlatformService {
  static Future<void> exportIncidentsToCSV(List<Incident> incidents,
    ...
    final List<int> bytes = [0xEF, 0xBB, 0xBF, ...contentBytes];

    // Guardamos el archivo físico temporalmente en el dispositivo
    final tempDir = await getTemporaryDirectory();
    final now = DateTime.now();
    final String timestamp =
        '${now.year}${now.month.toString().padLeft(2, '0')}${now.day.toString().padLeft(2, '0')}_${now.hour.toString().padLeft(2, '0')}${now.minute.toString().padLeft(2, '0')}';
    final String filePath =
        '${tempDir.path}/incidencias_urbiconnect_$timestamp.csv';

    final file = File(filePath);
    await file.writeAsBytes(bytes);

    // Activamos el selector nativo de compartir para guardar el archivo
    await Share.shareXFiles([XFile(filePath)],
        text: 'Exportando Colección de Incidencias CSV');
  }
  ...
  static Future<void> exportIncidentsToPDF(List<Incident> incidents,
    ...
    // Guardamos el archivo .html físicamente temporalmente en el dispositivo móvil
    final tempDir = await getTemporaryDirectory();
    final now = DateTime.now();
    final String timestamp =
        '${now.year}${now.month.toString().padLeft(2, '0')}${now.day.toString().padLeft(2, '0')}_${now.hour.toString().padLeft(2, '0')}${now.minute.toString().padLeft(2, '0')}';
    final String filePath =
        '${tempDir.path}/reporte_incidencias_$timestamp.html';

    final file = File(filePath);
    await file.writeAsBytes(bytes);

    // Activamos el selector nativo de compartir para guardar, imprimir o enviar el HTML
    await Share.shareXFiles([XFile(filePath)],
        text: 'Exportando Reporte de Incidencias');
  }
  ...
  static Future<void> exportStatsToPDF({
    ...
    final String timestamp =
        '${now.year}${now.month.toString().padLeft(2, '0')}${now.day.toString().padLeft(2, '0')}_${now.hour.toString().padLeft(2, '0')}${now.minute.toString().padLeft(2, '0')}';
    final String filePath =
        '${tempDir.path}/informe_estadistico_$timestamp.html';

    final file = File(filePath);
    await file.writeAsBytes(bytes);

    // Activamos el selector nativo de compartir para guardar, imprimir o enviar
    await Share.shareXFiles([XFile(filePath)],
        text: 'Exportando Informe Estadístico');
  }
